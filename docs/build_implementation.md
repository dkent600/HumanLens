...
**Silence vs exception (the trust boundary):** wiring for the principle now stated in `build_approach.md` ("Lens processing"). Bad model output — bad JSON, a fence, prose, wrong-typed fields, a *refusal* (mapped to `{text:''}`) — fails safe to **silence** (no findings, never fabricated, never a crash). A transport/API failure is different: it **propagates** as an exception (the SDK retries 429/5xx/network first), never disguised as silence. "Model misbehaved → silence; infrastructure failed → surfaced."
