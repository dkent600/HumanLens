...
The LLM provider seam stays `complete({system?, prompt}) → {text}`, domain-agnostic. `AnthropicLlmProvider` (`@anthropic-ai/sdk`, model `claude-opus-4-8` as a single named constant, Messages API, adaptive thinking) sits beside `FakeLlmProvider` as a second implementation. *As of this writing it is wired for Listening and Human Meaning.*
