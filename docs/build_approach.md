...
those remain matters of stochastic model output and human review. Completeness is
enforced in orchestration code (never in a prompt), by per-voice **fan-out** for
per-voice lenses and a cited-or-residual **audit** for cross-voice lenses — both
detailed below and in `build_implementation`.
...
- **Surface every voice, including the quiet or buried ones** — position must not suppress a voice. This is Listening's **completeness invariant**: every *authored* voice yields at least one finding, so a zero-finding authored voice is a provable error, not a judgment. Only genuinely non-authored emptiness yields none.
...
Because even an ungroundable answer still yields the worth-exploring flag, Human Meaning produces **at least one finding for every voice** — its completeness invariant. A voice with no finding is a provable error, never a silent drop; the flag is precisely what keeps that promise where meaning can't be read.
...
Completeness sits one layer beneath that boundary, and it is an **accounting property guaranteed in code, never asked of the model**. Every voice a lens is given ends in exactly one of four terminal states, matched by the orchestrator to an outcome and never inferred from the model's own claim to have been thorough: **answered-with-findings**; **answered-empty** — the model usably responded and genuinely surfaced nothing (a chosen silence, never retried); **delivered-but-unusable** — a refusal or malformed response, where the model did not usably answer but nothing broke (retryable at the model layer, then surfaced as a gap); and **failed** — the call could not complete (retryable at the infrastructure layer, then surfaced). There is no fifth state: a silently dropped voice is not a state a voice can be in — it is the hole this accounting closes; every voice reaches one of the four, or the run is not done. For the per-voice lenses (Listening, Human Meaning) this is realized by **fan-out** — one model call per voice, so the model never sees a list it could skip an item on. One subtlety carries the design's sharpest lesson: because a raw empty body cannot itself certify *which* empty it is — a refusal can arrive as empty text — answered-empty is separated from delivered-but-unusable by reading the response's finish signal, not its body alone; conflating them is the one way a dropped voice could masquerade as chosen silence. These four states are universal to every lens; a lens's own contract may *narrow* which it can legitimately reach — Listening reaches answered-empty only for non-authored emptiness, Human Meaning effectively never (its flag yields a finding even where meaning can't be read). Those narrowings are consequences of the lenses' own rules, not exceptions to the accounting. (Mechanics — the ledger, the four states, retry, termination, run-scoping — live in `build_implementation.md`.)

For the **cross-voice lenses** (the Aggregate wave onward — Culture Pattern, Tension, and the later waves that read across the finding set), completeness cannot take the same form: their work is to read the whole set at once and emit however many patterns exist, so there is no one-output-per-voice to check. The guarantee moves *after* the call instead. Every pattern a lens emits must cite the finding IDs it is built on, and orchestration code then subtracts cited from delivered; whatever was never cited lands in a **residual** the reviewer sees. The residual is computed in code, never a finding the lens is asked to emit — a model that silently drops a finding cannot be trusted to report having dropped it. A non-empty residual is **expected and correct**, not a failure: a finding no pattern absorbed is legitimately *not part of a pattern*, and may itself be an outlier worth a human's eye. So the cross-voice guarantee is not "the residual is empty" but "nothing is *silently* uncited — every finding is either woven into a pattern or visible in the residual." The audit and its coverage ratio are review diagnostics only: they are never stated to the model as a target, and the synthesis prompt never instructs exhaustive citation — findings cite what they genuinely use, and the residual honestly holds the rest. What the residual then asks of a human — the system proves delivery, not that each finding was weighed — is set out in plain terms just below.

**Completeness, both cases.**

*Per-voice lenses: prevent the drop.* Each voice gets its own model call. The model never sees a list, so it cannot skip an item on one. Our code holds the roster, sends one call per voice, and records one of four outcomes for each: findings, genuinely nothing, unusable response, or failure. The run is not done until every voice has an outcome. No fifth state exists; a voice cannot vanish.

*Cross-voice lenses: detect the drop.* Pattern-finding requires seeing all findings at once, so these lenses receive the complete, closed set from the prior wave in one call — the list is unavoidable, and so is the risk of skipping. The guarantee therefore moves after the call: every pattern must cite the IDs of the findings it is built on; our code then subtracts cited from sent. Whatever was never cited lands in a visible residual for the reviewer. The model can fail to use a finding; it cannot hide one.

*What the residual asks of a human.* The system can prove every finding was delivered to the lens. It cannot prove the model actually weighed each one. So an uncited finding is one of two things, and we can't tell which: considered and set aside, or overlooked. The reviewer's look is what settles it — that look is part of the completeness guarantee, not cleanup after it.

*The task:* for each uncited finding, ask does this matter? Either it's worth naming in the brief, or it shows the patterns missed something, or it's peripheral and stays set aside — now by a person's judgment instead of the model's silence.

Some findings will always go uncited; that alone means nothing. What matters is the share: the system shows what fraction of the findings ended up in the patterns and what fraction didn't. When most went uncited, don't start triaging item by item — first ask why. Skim a few uncited findings: if they're thin or beside this lens's question, the material simply didn't carry much, and the small pattern set is probably honest. If they're substantive and relevant yet uncited, the run under-delivered and should be redone rather than patched by hand. There's no fixed cutoff; that judgment belongs to the reviewer and sharpens with use.

*One sentence:* where the model cannot be given a list, no list exists to drop from; where it must be given one, our code makes everything it left uncited visible — and a human's look is what turns "set aside" into "considered."
...
- **When structural evaluation is automated.** Version 0 has no pipeline, so its checks are manual. The point at which the structural checks become automated regression guards depends on when the pipeline itself exists, and should be settled as part of the build sequence.
...
flowchart TD
    ...
    V1["V1 · THE TRUSTWORTHY ENGINE — full spine online<br/>• scoping (engagement + actor) on every record<br/>• de-identify gate (basic scan + human checkpoint)<br/>• staged lens pipeline — 7 lenses, 6 waves<br/>• Finding with enforced evidence anchoring<br/>• brief as ONE projection (client-safe ⊆ internal)<br/>• human review + capture<br/>• structural evaluation<br/>• auth/authz SEAMS present (resolve trivially: identity assumed, access granted)"]
