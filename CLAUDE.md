...
- **Identity** is resolved at the web layer once per request and threaded inward. **Authorization** is enforced by the **engine**: every **actor-facing operation** (intake, fetch brief, submit review, export, …) checks the authorization seam at its boundary, so the engine is self-protecting — internal pipeline steps do not each need their own check. Both seams resolve trivially in V1 (identity assumed; access always granted) but **deny is a first-class return value** (a small decision object with a reason) — every actor-facing call site branches and has a **tested deny path**.
...
- **Two-layer output:** client-safe ⊆ internal. The client-safe layer is a filtered, voice-calibrated projection of the internal finding set, with evidence links preserved. **Only client-safe types are exposed to the frontend** (via the `shared` client-safe contract); internal-layer types stay in the backend and must **never** be published from `shared` — that package boundary is what enforces client-safe ⊆ internal, so don't breach it for convenience.
...
- **The `docs/` files are read-only spec from your side.** They have a single writer (the SMI build/design chat), to keep one coherent source of truth. When a design decision changes — or you discover something that should update the spec — **do not edit `docs/` directly**; draft a short note (decision + rationale) and hand it back to be recorded there. The docs in `docs/` remain the source of truth, not chat history.
