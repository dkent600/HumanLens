...
// The findings these fixture comments synthesize into, with the default fake's emit
// behavior (one finding per emit lens): listening:0 + meaning:0 (Evidence), tension:0
// + culture:0 (Aggregate), objective:0 (Interpret), opening:0 (Openings).
const FIXTURE_PROMOTE: readonly string[] = ['listening:0', 'meaning:0', 'tension:0'];
const FIXTURE_SENSITIVE: readonly string[] = ['meaning:0'];
 ...
 *
 *   - promotes listening:0, meaning:0, tension:0;
 *   - flags meaning:0 sensitive — so it is promoted BUT held back by the sensitivity
 *     backstop at Assemble;
 *   - leaves culture:0, objective:0, opening:0 held (held by default).
 *
 * Result: the engine holds 6 findings internally; the client sees 2 (listening:0,
 * tension:0). The render visibly shows client-safe ⊊ internal — the client sees fewer
 * than the engine holds — rather than "everything came through".
 *
